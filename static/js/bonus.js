//DROPDOWN CREATION + REFRESH ABILITY//
function init(){
...
function Dashboardcontent(sample_name){
    d3.json("samples.json").then((data) => {
        ...
        let layout = {
            title: `Top 10 Bacteria Cultures Found in Sample ${selData.id}`,
            margin: {t:30, l:150}
        };
        ...
        let layout_2 ={
            title: `Bacteria Cultures for Sample ID ${selData.id}`
        };
...
function Bonus(sample_name){
    ...
    d3.json("samples.json").then((data) => {
        ...
        let layout_3 ={
            title: `Belly Button Washing Frequency<br>Scrubs per Week for Sample ID ${selData.id}`,
            shapes:[{
                type: 'path',
                path: path,
